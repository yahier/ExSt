...
import com.stbl.stbl.barcoe.camera.CameraManager;
import com.stbl.stbl.barcoe.decode.BitmapDecoder;
import com.stbl.stbl.barcoe.decode.CaptureActivityHandler;
import com.stbl.stbl.barcoe.view.ViewfinderView;
...
public final class CaptureActivity extends BaseActivity implements SurfaceHolder.Callback, View.OnClickListener {
    ...
    public static final String TagIsScanNoAction = "isScanNoAction";
    //如果此值是true,那么就将结果返回给来源页面，而不做网页跳转。
    private boolean isScanNoAction = false;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case PARSE_BARCODE_SUC: // 解析图片成功
                    LogUtil.logE(TAG, msg.obj.toString());
                    CaptureActivity activity = (CaptureActivity) activityReference.get();
                    if (activity != null) {
                        activity.afterHandle(msg.obj.toString());
                    }
                    break;

                case PARSE_BARCODE_FAIL:// 解析图片失败
                    ToastUtil.showToast(R.string.no_qr_code_found);
                    break;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isScanNoAction = getIntent().getBooleanExtra(TagIsScanNoAction, false);
        hasSurface = false;
        ...
        tvFlashLight = (TextView) findViewById(R.id.capture_flashlight);
    ...
    protected void onPause() {
        ...
        tvFlashLight.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.icon_scan_flash, 0, 0);
        ...
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                case REQUEST_CODE:
                    ...
                    try {
                        imgBytes = BitmapUtil.readStream(resolver.openInputStream(Uri.parse(originalUri.toString())));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    ...
                    final Bitmap bm = BitmapUtil.getPicFromBytes(imgBytes, opts);
...
//                            Bitmap img = BitmapUtils.getCompressedBitmap(photoPath);
    ...
    /**修改此处跳转规则，请一起修改ImagePagerAct的afterHandle方法*/
    private void afterHandle(String result) {
        if (isScanNoAction) {
            if (!result.contains(String.valueOf(CommonShare.sharedMiTribe))&&!result.contains(String.valueOf(CommonShare.sharedShoutu))) {
                ToastUtil.showToast(R.string.please_scan_friend_qr);
                restartPreviewAfterDelay(0L);
                return;
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.capture_scan_photo: // 图片识别
                ...
                Intent wrapperIntent = Intent.createChooser(innerIntent, getString(R.string.choose_qr_img));
            ...
            case R.id.capture_flashlight:
                if (isFlashlightOpen) {
                    ...
                    tvFlashLight.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.icon_scan_flash_pressed, 0, 0);

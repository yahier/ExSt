...
import com.stbl.stbl.ui.DirectScreen.widget.AudioMultiMembersControlsWidget.OnAudioMultiMembersClickListener;
...
class AVEndpointControl {
	...
	private OnAudioMultiMembersClickListener mMembersOnClickListener = new OnAudioMultiMembersClickListener() {
			...
			if(!mIsSupportMultiView){
				...
				if(hasRequest){
					mContext.sendBroadcast(new Intent(
							QavsdkContacts.ACTION_VIDEO_CLOSE).putExtra(
							QavsdkContacts.EXTRA_IDENTIFIER, mRequestIdentifierList[index]).putExtra(
							QavsdkContacts.EXTRA_VIDEO_SRC_TYPE, mRequestViewList[index].videoSrcType));
					...
					*/
					...
					AVEndpoint.requestViewList(mRequestIdentifierList, mRequestViewList, mRequestCount, mRequestViewListCompleteCallback);
					mContext.sendBroadcast(new Intent(
							QavsdkContacts.ACTION_VIDEO_SHOW).putExtra(
							QavsdkContacts.EXTRA_IDENTIFIER, identifier).putExtra(
							QavsdkContacts.EXTRA_VIDEO_SRC_TYPE, view.videoSrcType));	
				}
			}
		}
	 ...
	 *            成员列表界面
	 * */
	void initMembersUI(AudioMultiMembersControlsWidget membersUI) {
		mRemoteHasVideo = false;// 对方是否有视频画面
		mRemoteVideoIdentifier = "";
		Resources resources = mContext.getResources();
		membersUI.setOnAudioMultiMembersClickListener(mMembersOnClickListener);
	...
	public void closeRemoteVideo() {
		...
		mContext.sendBroadcast(new Intent(QavsdkContacts.ACTION_VIDEO_CLOSE));
	}

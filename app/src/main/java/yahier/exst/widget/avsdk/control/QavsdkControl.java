...
public class QavsdkControl {
	...
	public int startContext(String identifier, String usersig) {
		if (mAVContextControl == null)
			return QavsdkContacts.DEMO_ERROR_NULL_POINTER;
	...
	public void onCreate(Context context, View contentView) {
		...
		mAVEndpointControl.initMembersUI((AudioMultiMembersControlsWidget) contentView.findViewById(R.id.audio_place_widget));
	}

	public void onResume() {
		AVContext avContext = mAVContextControl.getAVContext();
		if (avContext != null)
			avContext.onResume();
	...
	public void onPause() {
		...
		if(mAVContextControl != null){
			AVContext avContext = mAVContextControl.getAVContext();
			if (avContext != null) avContext.onPause();
		}

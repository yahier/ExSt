...
class AVRoomControl {
	...
	private AVRoomMulti.Delegate mRoomDelegate = new AVRoomMulti.Delegate() {
		...
		public void onEnterRoomComplete(int result) {
			...
			mContext.sendBroadcast(new Intent(QavsdkContacts.ACTION_ROOM_CREATE_COMPLETE).putExtra(QavsdkContacts.EXTRA_AV_ERROR_RESULT, result));
		}
		...
		public void onExitRoomComplete(int result) {
			...
			mContext.sendBroadcast(new Intent(QavsdkContacts.ACTION_CLOSE_ROOM_COMPLETE));			
		}
/*
...
*/
		...
		public void onChangeAuthority(int retCode) {
			...
			mContext.sendBroadcast(new Intent(QavsdkContacts.ACTION_CHANGE_AUTHRITY).putExtra(QavsdkContacts.EXTRA_AV_ERROR_RESULT, retCode));
		}
	};
	...
	private void onMemberChange(int eventid, String[] updateList) {
		...
		mContext.sendBroadcast(new Intent(QavsdkContacts.ACTION_MEMBER_CHANGE));
	}
	...
	void enterRoom(int relationId, String roomRole) {
		...
		AVRoom.EnterRoomParam enterRoomParam = new AVRoomMulti.EnterRoomParam(relationId, QavsdkContacts.auth_bits, authBuffer, roomRole, audioCat);
		// create room
		if(avContext != null ) avContext.enterRoom(AVRoom.AV_ROOM_MULTI, mRoomDelegate, enterRoomParam);

...
import com.stbl.stbl.item.ChatEmoji;
...
public class FaceConversionUtil {
	...
	private static final int widthHeight = DimenUtils.sp2px(14);
	...
	public SpannableString addFace(Context context, int imgId, String spannableString) {
		//widthHeight = context.getResources().getDimension(R.dimen.emoji_width_height);
		//widthHeight =context.getResources().getDimensionPixelSize(R.dimen.emoji_width_height);
		//widthHeight=80;
		if (TextUtils.isEmpty(spannableString)) {
			return null;
		}
		Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), imgId);
		//Log.e("addFace1", bitmap.getWidth()+":"+bitmap.getHeight()); //48  96
		bitmap = Bitmap.createScaledBitmap(bitmap, widthHeight,widthHeight, true);
		Log.e("addFace2", bitmap.getWidth()+":"+bitmap.getHeight());//200  400
	...
	private void dealExpression(Context context, SpannableString spannableString, Pattern patten, int start) throws Exception {
		//widthHeight = (int) context.getResources().getDimension(R.dimen.emoji_width_height);
		//widthHeight =context.getResources().getDimensionPixelSize(R.dimen.emoji_width_height);
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				Log.e("dealExpression1", bitmap.getWidth()+":"+bitmap.getHeight());  //96
				bitmap = Bitmap.createScaledBitmap(bitmap, widthHeight,widthHeight, true);//50
				//Log.e("dealExpression2", bitmap.getWidth()+":"+bitmap.getHeight()); //400
				// 通过图片资源id来得到bitmap，用一个ImageSpan来包装
				//ImageSpan imageSpan = new ImageSpan(bitmap);
				ImageSpan imageSpan = new ImageSpan(context, bitmap);//代替上面的一行 by yahier
	...
	private void ParseData(List<String> data, Context context) {
		...
		try {
			...
			//ToastUtil.showToast(context, "表情size:"+emojiLists.size());
		} catch (Exception e) {
			e.printStackTrace();

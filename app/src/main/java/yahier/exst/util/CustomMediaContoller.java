...
import com.stbl.stbl.util.media.IMediaController;
...
public class CustomMediaContoller implements IMediaController {
    ...
    final String TAG = "CustomMediaContoller";
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SET_VIEW_HIDE:
                    ...
                    if (onShowBackListener != null){
                        onShowBackListener.show(false);
                    }
                    break;
    ...
    private void initAction() {
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                String string = generateTime((long) (duration * progress * 1.0f / progressMax));
                time.setText(string);
            }
            ...
            public void onStartTrackingTouch(SeekBar seekBar) {
                ...
                LogUtil.logE(TAG,"onStartTrackingTouch");
                show();
            ...
            public void onStopTrackingTouch(SeekBar seekBar) {
                isDragging = false;
                videoView.seekTo((int) (duration * seekBar.getProgress() * 1.0f / progressMax));
                handler.removeMessages(MESSAGE_SHOW_PROGRESS);
                audioManager.setStreamMute(AudioManager.STREAM_MUSIC, false);
                isDragging = false;
                handler.sendEmptyMessageDelayed(MESSAGE_SHOW_PROGRESS, 1000);
                LogUtil.logE(TAG,"onStopTrackingTouch");
                show();
            }
        });
...
//                     Intent intent = new Intent(view.getContext(), VideoPlayAct.class);
...
//                }
    ...
    OnListner onListener;

    public void setOnListener(OnListner onListener) {
    ...
    private String generateTime(long time) {
       // LogUtil.logE("controller","generateTime");
        ...
        if (seekBar != null) {
            if (duration > 0) {
               // long pos = 100L * position / duration;
                long pos = progressMax * position / duration;

                seekBar.setProgress((int) pos);
                //LogUtil.logE("setProgress pos:"+pos+"   position:"+position+"  duration:"+duration);
            }
            int percent = videoView.getBufferPercentage();
            seekBar.setSecondaryProgress(percent);
        }
        String string = generateTime((long) (duration * seekBar.getProgress() * 1.0f / progressMax));
        //LogUtil.logE("setProgress time:"+duration+":"+seekBar.getProgress());
        time.setText(string);
        return position;
    }
    ...
    VideoPlayView.OnShowBackListener onShowBackListener;
    public void setOnShowBackListener(VideoPlayView.OnShowBackListener onShowBackListener){
        this.onShowBackListener = onShowBackListener;
    }
}

...
public class NetworkRequestUsingHttpDNS {
    ...
    private static final String TEST_URL = "dev-api.stbl.cc";
    final  static String tag = "NetworkRequestUsingHttpDNS";
    public static void main(final Context ctx) {
        try {
            LogUtil.logE(tag,"main开始");
            // 设置APP Context和Account ID，并初始化HTTPDNS
            httpdns = HttpDns.getService(ctx, Config.aliyunAccountId);
            ...
            // 设置预解析域名列表
            httpdns.setPreResolveHosts(new ArrayList<>(Arrays.asList(TEST_URL)));
            ...
            if (ip != null) {
                ...
                LogUtil.logE(tag, "Get IP: " + ip + " for host: " + url.getHost() + " from HTTPDNS successfully!");

...
public class ScoreDetailAdapter extends BaseAdapter {
    ...
    public ScoreDetailAdapter(Context context, ArrayList<Currency> list) {
        mList = list;
    ...
    private void bindViewHolder(ViewHolder holder, int position) {
        ...
        if (currency.getOptype() == 0) {
            ...
            holder.mAmountTv.setTextColor(mContext.getResources().getColor(R.color.theme_red));
        } else {
            holder.mAmountTv.setText("- " + ((int) currency.getAmount()));
            holder.mAmountTv.setTextColor(mContext.getResources().getColor(R.color.theme_yellow_e38));
        }
